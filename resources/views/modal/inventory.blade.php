    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="inventoryPartnerForm" action="{{ route('partner.inventory.store') }}" method="post">
                    @csrf
                    @php
                    ...
                    @endphp
                    <input type="hidden" name="brand_id" id="brand_id" value="{{ App\User::find(Auth::user()->id)->brands->id }}" />
                    <input type="hidden" name="brand_code" id="brand_code" value="{{ App\User::find(Auth::user()->id)->brands->brand_code }}" />
                    <div class="form-row">
                        <h5 class="col-12">Item Information</h5>
                        <div class="form-group col-md-4 col-xs-12">
                            <label>Product Name</label>
                            <input type="text" class="form-control" name="name" placeholder="Enter brand name.." id="name" required/>
                        </div>
                        <div class="form-group col-md-4 col-xs-12">
                            <label>Brand Partner</label>
                            <input type="text" class="form-control" id="brand_name" value="{{ App\User::find(Auth::user()->id)->brands->name }}" readonly/>
                        </div>
                        <div class="form-group col-md-4 col-xs-12">
                        ...
                        <div class="form-group col-md-4 col-xs-12">
                            ...
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text">{{ App\User::find(Auth::user()->id)->brands->brand_code }}</span>
                                </div>
                                <input type="text" class="form-control" value="{{ $itemCount }}" readonly/>
                                <input type="hidden" class="form-control" name="sku" id="sku" value="{{ App\User::find(Auth::user()->id)->brands->brand_code }}{{ $itemCount }}"/>
                            </div>
                        </div>
                        <div class="form-group col-md-4 col-xs-12">
                            <label>Re-stock Point</label>
                            <input type="text" class="form-control num-only" name="restock" id="restock" value="0" required/>
                        </div>
                        <div class="form-group col-md-4 col-xs-12">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control num-only" name="price" id="price" required/>
                            </div>
                        </div>
                        <div class="form-group col-md-12 col-xs-12">
                            <div class="clearfix text-right mt-3">
                                <button type="submit" id="inventoryPartnerBtn" class="btn btn-success">
                                    Add Item
                                </button>
                            </div>
                        </div>
